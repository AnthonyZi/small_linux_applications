...
do
  ...
  min_density_x=$(echo "${img_size_x}/${xfactor}*1.1"  | bc -l);
  min_density_y=$(echo "${img_size_y}/${yfactor}*1.1" | bc -l);
  ...
  page_size_x=$(echo "${xfactor}*${density}" | bc -l);
  page_size_y=$(echo "${yfactor}*${density}" | bc -l);
